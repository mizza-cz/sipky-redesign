...
(function () {
  ...
  const isBranch = (li) => li && li.classList && li.classList.contains('with-sub');
  ...
  const closeSiblings = (li) => {
    ...
    Array.from(parentUl.children).forEach((sib) => {
      if (sib !== li && sib.classList && sib.classList.contains('with-sub')) {
        sib.classList.remove('open');
        // закрыть всё вложенное внутри соседа
        sib.querySelectorAll('.with-sub.open').forEach((x) => x.classList.remove('open'));
      }
    });
  };
  ...
  sidebar.addEventListener('click', (e) => {
    ...
    const li =
      link.closest('li.nav-sub-item.with-sub') || link.closest('li.sidebar-nav-item.with-sub');
  ...
  const links = sidebar.querySelectorAll('a.sidebar-nav-link, a.nav-sub-link');
