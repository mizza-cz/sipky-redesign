$(function () {
  ...
  var path = loc.split('/');
  var isRtl = path[3].indexOf('rtl') >= 0 ? true : false; // path[2] for production
  var isSidebar = path[3].indexOf('sidebar') >= 0 ? true : false; // path[2] for production
  var newloc = '';
  ...
  $('body').on('click', '.slim-direction', function () {
    ...
    if (val === 'rtl') {
      if (!isRtl) {
        if (path[4]) {
          newloc = '/demo/slim1.1/' + path[3] + '-rtl/' + path[4];
        } else {
          newloc = '/demo/slim1.1/' + path[3] + '-rtl/';
        }
        $(location).attr('href', newloc);
      }
    } else {
    ...
    if (val === 'vertical') {
      if (!isSidebar) {
        if (loc.indexOf('-dark') >= 0) {
          if (path[4]) {
            newloc = '/demo/slim1.1/sidebar-dark/' + path[4];
          } else {
            newloc = '/demo/slim1.1/sidebar-dark/';
          }
        } else {
          if (path[4]) {
            newloc = '/demo/slim1.1/sidebar/' + path[4];
          } else {
            newloc = '/demo/slim1.1/sidebar/';
          }
        }
        $(location).attr('href', newloc);
      }
      ...
      if (isSidebar) {
        if (path[4]) {
          newloc = '/demo/slim1.1/' + path[3].replace('sidebar', 'template') + '/' + path[4];
        } else {
          newloc = '/demo/slim1.1/' + path[3].replace('sidebar', 'template') + '/';
        }
        $(location).attr('href', newloc);
      }
    }
